...
@Repository
public interface SrcPdsPOCRepo extends JpaRepository<SrcPdsPOC, Long> {

    @Query(value = "SELECT ufc FROM public.pds_07_jul_2023_poc where uid = :uid  ;",nativeQuery = true)
    String getUfc(String uid);

    @Query(value = "SELECT makkal_number FROM public.pds_07_jul_2023_poc where uid = :uid  ;",nativeQuery = true)
    String getMkklId(String uid);

    @Query(value = "SELECT uid FROM public.pds_07_jul_2023_poc where makkal_number = :mkklId  ;",nativeQuery = true)
    String getAadhaarId(String mkklId);

    @Query(value = "SELECT name_in_english as nameEng, name_in_tamil as nameTam, sex, address_line1 as addressL1, address_line2 as addressL2, address_line3 as addressL3, village_name as villageName, taluk_name as talukName, district_name as districtName, pincode, l_address_line1 as lAddress1, l_address_line2 as lAddress2, l_address_line3 as lAddress3, l_village_name as lVillage, l_taluk_name as lTaluk, l_district_name as lDistrictName, makkal_number as makkalNum FROM public.pds_07_jul_2023_poc where uid = :uid ;", nativeQuery = true)
